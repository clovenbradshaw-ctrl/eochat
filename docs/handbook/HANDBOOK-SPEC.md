...
**Status:** draft v0.1, structure and scope only. No chapter content is written
...
- **3.6 How the engine is allowed to grow.** The growth rule — "unwired is
  failing," nothing is ported, every organ is re-earned — and a short
  worked example (Amendment XIV/XV, the turbulence candidate) of a proposal
  actually being tested and refused, to show the rule has teeth.

  *Sources:* `eoreader6/SEED.md` (Amendments I–XX, selectively), `eochat/
  essay.md` as the worked example, `eo-constitution/README.md`'s domain map
  for organ vocabulary.
...
- **6.2 The fifty-year-old objection nobody has fully answered.** Fillmore's
  case-role problem, told simply: the same sentence position can mean
  different things ("the hammer broke the window" vs. "John broke the
  window"), and why that's still a live, unsolved edge for this project
  too — stated as an honest limitation, not glossed over.
- **6.3 What's actually new here.** Typed refusal as a normal output,
  ...
  statistics instead of fixed thresholds, and a growth rule that has
  actually refused its own author's best result.
- **6.4 The honest gap list.** What Part III's organs don't yet do
  ...
  dimensions) — stated the way the source material states it: as a map of
  what's next, not a flaw to hide.

  *Sources:* `eoreader6/prior-art-teachable-language-comprehender.md` in
  full — this Part is close to a direct, de-jargonized translation of that
  essay, which is already written in an unusually accessible voice for a
  primary source.
